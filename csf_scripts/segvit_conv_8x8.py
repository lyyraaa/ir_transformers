...
r_method = 'linear'  # {'linear','pca,'fixed'} #todo
reduce_dim = 64 if is_local else int(sys.argv[-1])  # todo
...
class ConvEncoderBlock(torchvision.models.vision_transformer.EncoderBlock):
    def __init__(self, d_model, expand_dim, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.conv_expand = nn.Conv2d(d_model, expand_dim, kernel_size=3, stride=1, padding=1)
        self.conv_contract = nn.Conv2d(expand_dim, d_model, kernel_size=3, stride=1, padding=1)
        self.activation = nn.ReLU()

    def mlp(self, y):
        y = self.tok_to_img(y)
        y = self.conv_contract(self.activation(self.conv_expand((y))))
        y = self.img_to_tok(y)
        return y

    def tok_to_img(self, x):
        b, tok, dmod = x.shape
        h = w = int(tok ** 0.5)
        # X : b x token x d_model -> b x c x h x w
        return x.permute(0, 2, 1).reshape(b, dmod, h, w)

    def img_to_tok(self, x):
        b, c, h, w = x.shape
    ...
    def forward(self, input: torch.Tensor):
        ...
        y = self.ln_2(x)  # todo does this need to change to batch norm or smth
        y = self.mlp(y)
        return x + y


class ConvEncoder(torchvision.models.vision_transformer.Encoder):
    def __init__(self,
                 seq_length: int,
                 num_layers: int,
                 num_heads: int,
...
class SegViT(torchvision.models.VisionTransformer):
    def __init__(
        ...
        self.heads = nn.Sequential(
            nn.Conv2d(self.hidden_dim + (len(wavenumbers_used) if r_method == 'fixed' else reduce_dim),
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.permute(0, 2, 1).reshape(b, self.hidden_dim, hpatch, wpatch)
        x = self.upscale(x)
...
for r in range(model.encoder.pos_embedding.shape[1] - 1):
    for c in range(model.encoder.pos_embedding.shape[1] - 1):
        locarray[r, c] = cos(model.encoder.pos_embedding[0][r + 1],
                             model.encoder.pos_embedding[0][c + 1]).detach().cpu().numpy()
