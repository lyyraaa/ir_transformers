...
valid_reduce_order = [1, 3, 7, 15, 31, 63, 127, 255, 511, 1023]
reduce_order = 7 if is_local else int(sys.argv[-3])

splits = np.linspace(0, wv_end, reduce_order + 2)[1:-1]
splits = np.unique(np.rint(splits)).astype(int)
if len(splits) == 964: splits = np.concatenate([[0, ], splits])

channels_used = np.s_[..., splits]
